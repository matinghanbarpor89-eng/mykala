...
/* USERS */
const users = [
  {email:"user1@test.com", password:"1234", name:"کاربر ۱"},
  {email:"user2@test.com", password:"abcd", name:"کاربر ۲"}
];
  ...
  list.forEach(p => {
    container.innerHTML += `
      <div class="card">
        ...
        <button onclick="addToCartUser(${p.id})">افزودن به سبد</button>
      </div>
    `;
  });
}
...
function loadCartUser() {
  const cart = getCartUser();
  const div = document.getElementById("cartItemsUser");
  let total = 0;
  div.innerHTML = "";
  cart.forEach((p, i) => {
    total += p.price;
    div.innerHTML += `
      <div class="cart-item">
        <span>${p.name}</span>
        <span>${p.price.toLocaleString()} <button onclick="removeFromCartUser(${i})">❌</button></span>
      </div>
    `;
  });
  document.getElementById("totalPriceUser").innerText = "جمع کل: " + total.toLocaleString() + " تومان";
}
...
function placeOrder() {
  ...
  if(!name || !phone || !address || cart.length === 0) {
    showAlert("تمام فیلدها و سبد خرید باید پر شود");
    return;
  }
...
function login() {
  ...
  if(user === adminUserName && pass === adminPassword) {
    showAlert("ورود مدیر موفق!");
    showPage("admin");
  } else {
    const u = users.find(x => x.email === user && x.password === pass);
    if(u) {
      showAlert("ورود کاربر موفق!");
      showPage("user");
    } else {
...
function renderAdmin() {
  ...
  orders.forEach(o => {
    const items = o.items.map(it => it.name + " (" + it.price.toLocaleString() + " تومان)").join(", ");
    adminOrders.innerHTML += `
...
function addProduct() {
  ...
  if(!name || !price || !img) { showAlert("همه فیلدها را پر کنید"); return; }
...
function editProduct(i) {
  ...
  if(newName && newPrice && newImg) {
    products[i] = {id: p.id, name: newName, price: newPrice, img: newImg};
    renderProducts(products, productList);
    renderProducts(products, userProductList);
    renderAdmin();
  }
